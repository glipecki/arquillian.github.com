...
Nowa klasa testowa zawierająca deskryptor osadzania i metodę testową została dodana do projektu.
...
Ponownie utworzyliśmy nowy test, jednak tym razem dodatkowo wygenerowaliśmy persistence.xml w deskryptorze osadzania. Tak wygląda wygenerowany test:
...
p. Otwórz test w swoim IDE i dodaj encję @Language@ do deskryptora osadzania. Dodaj odpowiedni import i zmień linię:
...
h3. Wyeksportuj deskryptor osadzania

Dla celów debugowania wartościowe może się okazać eksportowanie deskryptora osadzania do pliku. Możesz to zrobić korzystając z Forge:

Przejdź do testu Arquilliana:

bc(command). $ cd src/test/java/org/arquillian/example/MySimpleBeanTest.java

Wyeksportuj deskryptor osadzania:
